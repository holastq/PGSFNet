...
def get_relative_coords_table_all(
    ...
    # get relative_coords_table
    ws = window_size#[x,x]
    aws = [w // anchor_window_down_factor for w in window_size]#achor window size:[y,y](y = x/s)
    pws = pretrained_window_size
    paws = [w // anchor_window_down_factor for w in pretrained_window_size]

    # positive table size: (Ww - 1) - (Ww - AWw) // 2 
    ts_p = [w1 - 1 - (w1 - w2) // 2 for w1, w2 in zip(ws, aws)]#[p,p](p = x - 1 - (x - y) // 2)
    # negative table size: -(AWw - 1) - (Ww - AWw) // 2
    ts_n = [-(w2 - 1) - (w1 - w2) // 2 for w1, w2 in zip(ws, aws)]#[n,n](n = -(y - 1) - (x - y) // 2)
    pts = [w1 - 1 - (w1 - w2) // 2 for w1, w2 in zip(pws, paws)]

    # TODO: pretrained window size and pretrained anchor window size is only used here.
    # TODO: Investigate whether it is really important to use this setting when finetuning large window size
    # TODO: based on pretrained weights with small window size. （finetuning）

    coord_h = torch.arange(ts_n[0], ts_p[0] + 1, dtype=torch.float32)# n n+1 n+2 ... p-2 p-1 p p+1 
    coord_w = torch.arange(ts_n[1], ts_p[1] + 1, dtype=torch.float32)# n n+1 n+2 ... p-2 p-1 p p+1 
    table = torch.stack(torch.meshgrid([coord_h, coord_w], indexing="ij")).permute(1, 2, 0)

    table = table.contiguous().unsqueeze(0)  # (1, x+y-1, x+y-1, 2)
    if pts[0] > 0:
        table[:, :, :, 0] /= pts[0]#h
        table[:, :, :, 1] /= pts[1]#w
    else:
    ...
    table = torch.sign(table) * torch.log2(torch.abs(table) + 1.0) / np.log2(8)
    return table
    # table_size:(1, x_ws_0 + y_aws_0-1, x_ws_1 + y_aws_1-1, 2)
...
def coords_diff_odd(coords1, coords2, start_coord, max_diff):
    ...
    # print(coords1.shape)
    # print(coords2.shape)
    coords = coords1[:, :, None] - coords2[:, None, :] 
    # [2, Wh*Ww, None] - [2, None, AWh*AWw] = [2, Wh*Ww, AWh*AWw]
    coords = coords.permute(1, 2, 0).contiguous()  # [Wh*Ww, AWh*AWw, 2]
    coords[:, :, 0] += start_coord[0] 
...
def get_relative_position_index_simple(
    ...
    coords = _get_meshgrid_coords((0, 0), window_size)#（2, Wh*Ww）
    coords_anchor = _get_meshgrid_coords((0, 0), aws) #（2，aWh*aWw）
